...
vmangos_default_branch() {
  if vmangos_is_main_target "${1:-$EXPANSION}"; then
    printf '%s' "$DEFAULT_VMANGOS_BRIDGE_BRANCH"
  else
    printf '%s' "$DEFAULT_VMANGOS_AHBOT_BRANCH"
  fi
}
...
edit_vmangos_source_pin() {
  ...
  if vmangos_is_main_target "$EXPANSION"; then
    echo
    echo "$(expansion_title "$EXPANSION") is pinned to the bridge build."
    echo "  Layer: $(vmangos_build_lane_label "$EXPANSION")"
    echo "  Repo:  ${DEFAULT_REPO}"
    echo "  Branch:${DEFAULT_BRANCH}"
    echo
    echo "Main is intentionally fixed so bridge validation stays consistent."
    read -p "Press Enter to continue..." _
    return 0
  fi
