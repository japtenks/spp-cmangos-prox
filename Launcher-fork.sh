...
declare -A GAME_HOSTNAMES
WEBSITE_REPO="${DEFAULT_WEBSITE_REPO}"
...
normalize_config_env() {
  ...
  append_config_default_line "GAME_INSTANCE_NAMES" '()'
  append_config_default_line "CLASSIC_CONTAINER_NAME" '"classic"'
  append_config_default_line "CLASSIC_CONTAINER_NUMBER" '"1"'
  append_config_default_line "TBC_CONTAINER_NAME" '"tbc"'
  ...
  append_config_default_line "VMANGOS_GIT_BRANCH" "\"${DEFAULT_VMANGOS_GIT_BRANCH}\""
  append_config_default_line "VMANGOS_DB_HOST" '""'
  append_config_default_line "VMANGOS_DB_PORT" '"3306"'
  append_config_default_line "VMANGOS_WORLD_DB_URL" '""'
  append_config_default_line "VMANGOS_DATA_PACK_URL" '""'
  append_config_default_line "CRASH_SHARE_ROOT" "\"${DEFAULT_CRASH_SHARE_ROOT}\""
  append_config_default_line "WEBSITE_REPO" "\"${DEFAULT_WEBSITE_REPO}\""
  ...
  append_config_default_line "LAUNCHER_FORK_NAME" '"Launcher-fork.sh"'
}
...
launcher_repo_clone_url() {
  ...
  printf '%s' "https://github.com/japtenks/spp-cmangos-prox.git"
}
...
update_launcher_self() {
  ...
  exec bash "$SCRIPT_DIR/Launcher-fork.sh"
}
...
auto_detect_stack() {
  ...
  local _exp _ct _vm_target _vm_host
  migrate_game_instance_metadata
  GAME_CTIDS=()
  GAME_HOSTNAMES=()
  for _exp in classic tbc wotlk; do
    _vm_host="spp-$(shared_game_target_name "$_exp")"
    _ct=$(awk -v name="$_vm_host" '$3==name {print $1}' <<< "$_pct") || true
    if [[ -z "$_ct" ]]; then
      _ct=$(awk -v name="spp-$_exp" '$3==name {print $1}' <<< "$_pct") || true
      [[ -n "$_ct" ]] && _vm_host="spp-$_exp"
    fi
    [[ -n "$_ct" ]] && GAME_CTIDS[$_exp]=$_ct
    [[ -n "$_vm_host" ]] && GAME_HOSTNAMES[$_exp]="$_vm_host"
  done
  while IFS= read -r _vm_target; do
    [[ -n "$_vm_target" ]] || continue
    _vm_host="spp-${_vm_target}"
    _ct=$(awk -v name="$_vm_host" '$3==name {print $1}' <<< "$_pct") || true
    if [[ -z "$_ct" && -z "${VMANGOS_REALM_OWNER:-}" ]]; then
      _ct=$(awk '$3=="spp-vmangos" {print $1}' <<< "$_pct") || true
      [[ -n "$_ct" ]] && _vm_host="spp-vmangos"
    fi
    [[ -n "$_ct" ]] && GAME_CTIDS[$_vm_target]=$_ct
    [[ -n "$_vm_host" ]] && GAME_HOSTNAMES[$_vm_target]="$_vm_host"
  done < <(vmangos_target_list)
  if [[ -n "${VMANGOS_REALM_OWNER:-}" && -n "${GAME_CTIDS[$VMANGOS_REALM_OWNER]:-}" ]]; then
    GAME_CTIDS[vmangos]="${GAME_CTIDS[$VMANGOS_REALM_OWNER]}"
    GAME_HOSTNAMES[vmangos]="${GAME_HOSTNAMES[$VMANGOS_REALM_OWNER]}"
  fi
  return 0
}
...
CLASSIC_CONTAINER_NUMBER="${CLASSIC_CONTAINER_NUMBER:-1}"
TBC_CONTAINER_NAME="${TBC_CONTAINER_NAME:-tbc}"
TBC_CONTAINER_NUMBER="${TBC_CONTAINER_NUMBER:-1}"
WOTLK_CONTAINER_NAME="${WOTLK_CONTAINER_NAME:-wotlk}"
WOTLK_CONTAINER_NUMBER="${WOTLK_CONTAINER_NUMBER:-1}"
VMANGOS_CONTAINER_BASENAME="${VMANGOS_CONTAINER_BASENAME:-vmangos}"
VMANGOS_INSTANCE_COUNT="${VMANGOS_INSTANCE_COUNT:-1}"
GAME_INSTANCE_NAMES=()
...
expansion_container_base_var_name() {
  case "$1" in
    classic) echo "CLASSIC_CONTAINER_NAME" ;;
    tbc) echo "TBC_CONTAINER_NAME" ;;
    wotlk) echo "WOTLK_CONTAINER_NAME" ;;
    *) return 1 ;;
  esac
}

expansion_container_number_var_name() {
  case "$1" in
    classic) echo "CLASSIC_CONTAINER_NUMBER" ;;
    tbc) echo "TBC_CONTAINER_NUMBER" ;;
    wotlk) echo "WOTLK_CONTAINER_NUMBER" ;;
    *) return 1 ;;
  esac
}

expansion_container_base_name() {
  local var_name
  var_name=$(expansion_container_base_var_name "$1") || return 1
  printf '%s' "${!var_name:-$1}"
}

expansion_container_number() {
  local var_name
  var_name=$(expansion_container_number_var_name "$1") || return 1
  printf '%s' "${!var_name:-1}"
}

shared_game_target_name() {
  local exp="$1"
  local base_name number
  base_name=$(expansion_container_base_name "$exp") || return 1
  number=$(expansion_container_number "$exp") || return 1
  printf '%s-%s' "$base_name" "$number"
}

default_vmangos_target_name() {
  local index="$1"
  printf '%s-%s' "${VMANGOS_CONTAINER_BASENAME:-vmangos}" "$index"
}

migrate_game_instance_metadata() {
  local changed=0
  local -a migrated_targets=()
  local name

  if declare -p GAME_INSTANCE_NAMES >/dev/null 2>&1 && [[ ${#GAME_INSTANCE_NAMES[@]} -gt 0 ]]; then
    return 0
  fi

  if declare -p VMANGOS_INSTANCE_NAMES >/dev/null 2>&1 && [[ ${#VMANGOS_INSTANCE_NAMES[@]} -gt 0 ]]; then
    for name in "${VMANGOS_INSTANCE_NAMES[@]}"; do
      case "$name" in
        main) migrated_targets+=("$(default_vmangos_target_name 1)") ;;
        ahbot) migrated_targets+=("$(default_vmangos_target_name 2)") ;;
        vmangos-*) migrated_targets+=("$name") ;;
        *) migrated_targets+=("${VMANGOS_CONTAINER_BASENAME:-vmangos}-${name}") ;;
      esac
    done
    changed=1
  fi

  if [[ ${#migrated_targets[@]} -eq 0 ]]; then
    local count="${VMANGOS_INSTANCE_COUNT:-1}"
    local i
    for ((i=1; i<=count; i++)); do
      migrated_targets+=("$(default_vmangos_target_name "$i")")
    done
    changed=1
  fi

  GAME_INSTANCE_NAMES=("${migrated_targets[@]}")
  VMANGOS_INSTANCE_COUNT="${#GAME_INSTANCE_NAMES[@]}"
  [[ -n "${VMANGOS_REALM_OWNER:-}" ]] || VMANGOS_REALM_OWNER="${GAME_INSTANCE_NAMES[0]}"

  if [[ $changed -eq 1 ]]; then
    persist_array_config_line "GAME_INSTANCE_NAMES" "${GAME_INSTANCE_NAMES[@]}"
    set_or_append_config_line "VMANGOS_INSTANCE_COUNT" "\"${VMANGOS_INSTANCE_COUNT}\""
    set_or_append_config_line "VMANGOS_REALM_OWNER" "\"${VMANGOS_REALM_OWNER}\""
    persist_config_storage || true
  fi
}
...
vmangos_target_list() {
  local target
  migrate_game_instance_metadata
  for target in "${GAME_INSTANCE_NAMES[@]}"; do
    [[ -n "$target" ]] && printf '%s\n' "$target"
  done
}
...
for EXP in vmangos $(vmangos_target_list); do
  for TYPE in WORLD CORE REALM CHARS LOGS MAPS WEBSITE; do
...
vmangos_is_main_target() {
  local target="${1:-$EXPANSION}"
  [[ "$target" == "${VMANGOS_REALM_OWNER:-}" || ( "$target" == "vmangos" && -n "${VMANGOS_REALM_OWNER:-}" ) ]]
}
...
vmangos_config_token() {
  ...
  [[ -n "$suffix" ]] || suffix="1"
  printf 'INSTANCE_%s' "$suffix"
}

vmangos_db_token() {
  local suffix
  suffix=$(vmangos_target_suffix "${1:-$EXPANSION}")
  if [[ -z "$suffix" || "$suffix" == "1" ]]; then
    printf 'vmangos'
    return 0
  fi
...
vmangos_instance_label() {
  local target="${1:-$EXPANSION}"
  if [[ "$target" == "vmangos" ]]; then
    target="${VMANGOS_REALM_OWNER:-vmangos}"
  fi
  echo "vMaNGOS ${target#*-}"
}

vmangos_default_repo() {
  printf '%s' "${VMANGOS_REPO_URL:-$DEFAULT_VMANGOS_REPO_URL}"
}

vmangos_default_branch() {
  printf '%s' "${VMANGOS_GIT_BRANCH:-$DEFAULT_VMANGOS_GIT_BRANCH}"
}
...
vmangos_repo_var_name() {
  echo "VMANGOS_REPO_URL"
}

vmangos_branch_var_name() {
  echo "VMANGOS_GIT_BRANCH"
}
...
vmangos_target_hostname() {
  local target="${1:-$EXPANSION}"
  if is_shared_classic_family "$target"; then
    echo "spp-$(shared_game_target_name "$target")"
    return 0
  fi
  if [[ "$target" == "vmangos" ]]; then
    target="${VMANGOS_REALM_OWNER:-$(default_vmangos_target_name 1)}"
  fi
  echo "spp-${target}"
}
...
EOF
  ...
  while IFS= read -r realm_exp; do
    [[ -n "$realm_exp" ]] || continue
    if [[ -n "${GAME_CTIDS[$realm_exp]:-}" || "$realm_exp" == "${EXPANSION:-}" ]]; then
      local realm_id world_db char_db
      realm_id=$(expansion_realm_id "$realm_exp") || continue
      world_db=$(vmangos_db_token "$realm_exp") || continue
      char_db="${world_db}characters"
      included=1
      cat <<EOF
    ${realm_id} => [
        'realmd' => '$(expansion_realm_db_name "${VMANGOS_REALM_OWNER:-$realm_exp}")',
        'world' => '${world_db}',
        'chars' => '${char_db}',
        'armory' => '',
        'bots' => '',
...
show_wotlk_wip_notice() {
  echo
  echo "WotLK is still a work in progress in Launcher-fork.sh."
  echo "The menu entry is intentionally stubbed so it stays visible without dropping into unfinished flows."
  echo "Planned build target: https://github.com/mod-playerbots/mod-playerbots"
...
resolve_shared_master_expansion() {
  ...
  if [[ -n "${SHARED_REALM_OWNER:-}" ]] && is_shared_classic_family "$SHARED_REALM_OWNER"; then
    echo "$SHARED_REALM_OWNER"
    return 0
  fi
...
derive_db_names() {
  ...
  elif [[ -n "${SHARED_REALM_OWNER:-}" ]] && is_shared_classic_family "$SHARED_REALM_OWNER"; then
    REALM_DB_NAME=$(expansion_realm_db_name "$SHARED_REALM_OWNER") || {
      echo "Unknown shared realm owner: $SHARED_REALM_OWNER"
      return 1
    }
...
pin_master_expansion() {
  if [[ -z "${MASTER_EXPANSION:-}" ]] || ! is_shared_classic_family "$MASTER_EXPANSION"; then
...
pin_master_realmd_db() {
  ...
  if [[ -z "${SHARED_REALM_OWNER:-}" ]]; then
    SHARED_REALM_OWNER="$EXPANSION"
    set_config_value "SHARED_REALM_OWNER" "$SHARED_REALM_OWNER"
    echo "Pinned shared realm owner: $SHARED_REALM_OWNER"
  fi

  if [[ -z "${MASTER_REALMD_DB:-}" ]]; then
    MASTER_REALMD_DB=$(expansion_realm_db_name "${SHARED_REALM_OWNER:-$EXPANSION}") || return 1
...
pin_vmangos_realm_owner() {
  ...
  if [[ -z "$owner" ]]; then
    owner="$EXPANSION"
    [[ "$owner" == "vmangos" ]] && owner="$(default_vmangos_target_name 1)"
    VMANGOS_REALM_OWNER="$owner"
    set_config_value "VMANGOS_REALM_OWNER" "$VMANGOS_REALM_OWNER"
    echo "Pinned vMaNGOS realm owner: $VMANGOS_REALM_OWNER"
  else
    echo "vMaNGOS realm owner already pinned: $VMANGOS_REALM_OWNER"
  fi
}
...
is_master() {
  [[ "${EXPANSION}" == "${SHARED_REALM_OWNER:-${MASTER_EXPANSION:-}}" ]]
}
...
prompt_game_container_metadata() {
  local target_expansion="$1"
  local base_name current_number new_base new_number

  echo
  if is_vmangos "$target_expansion"; then
    local current_count="${VMANGOS_INSTANCE_COUNT:-1}"
    echo "Current vMaNGOS base name: ${VMANGOS_CONTAINER_BASENAME:-vmangos}"
    echo "Current vMaNGOS instance count: ${current_count}"
    read -p "Game container base name [${VMANGOS_CONTAINER_BASENAME:-vmangos}]: " new_base
    read -p "Instance number [${current_count}]: " new_number
    new_base="${new_base:-${VMANGOS_CONTAINER_BASENAME:-vmangos}}"
    new_number="${new_number:-${current_count}}"
    [[ "$new_number" =~ ^[0-9]+$ ]] || return 1

    VMANGOS_CONTAINER_BASENAME="$new_base"
    if [[ "$new_number" -gt "${VMANGOS_INSTANCE_COUNT:-0}" ]]; then
      local i
      for ((i=${VMANGOS_INSTANCE_COUNT:-0}+1; i<=new_number; i++)); do
        GAME_INSTANCE_NAMES+=("$(default_vmangos_target_name "$i")")
      done
      VMANGOS_INSTANCE_COUNT="${#GAME_INSTANCE_NAMES[@]}"
      persist_array_config_line "GAME_INSTANCE_NAMES" "${GAME_INSTANCE_NAMES[@]}"
      set_or_append_config_line "VMANGOS_INSTANCE_COUNT" "\"${VMANGOS_INSTANCE_COUNT}\""
    fi
    set_config_value "VMANGOS_CONTAINER_BASENAME" "$VMANGOS_CONTAINER_BASENAME"
    EXPANSION="$(default_vmangos_target_name "$new_number")"
    [[ -n "${VMANGOS_REALM_OWNER:-}" ]] || set_config_value "VMANGOS_REALM_OWNER" "$EXPANSION"
    return 0
  fi

  base_name=$(expansion_container_base_name "$target_expansion") || return 1
  current_number=$(expansion_container_number "$target_expansion") || return 1
  read -p "Game container base name [${base_name}]: " new_base
  read -p "Instance number [${current_number}]: " new_number
  new_base="${new_base:-$base_name}"
  new_number="${new_number:-$current_number}"
  [[ "$new_number" =~ ^[0-9]+$ ]] || return 1

  local base_var number_var
  base_var=$(expansion_container_base_var_name "$target_expansion") || return 1
  number_var=$(expansion_container_number_var_name "$target_expansion") || return 1
  set_config_value "$base_var" "$new_base"
  set_config_value "$number_var" "$new_number"
  return 0
}
...
ensure_game_container() {
  ...
  prompt_game_container_metadata "$EXPANSION" || return 1
...
expansion_menu() {
  while true; do
    ...
    if [[ $INDEX -ge 0 && $INDEX -lt ${#ALLOWED_EXPANSIONS[@]} ]]; then
      ...
      if [[ "${ALLOWED_EXPANSIONS[$INDEX]}" == "wotlk" ]]; then
        show_wotlk_wip_notice
        continue
      fi
...
shared_services_menu() {
  ...
  while true; do
    ...
    echo "Classic/TBC/WotLK share the login container. vMaNGOS realmd stays on the first vMaNGOS game container."
    echo
    echo "1 - Status"
...
sync_sql_repo() {
  local repo_url
  repo_url=$(launcher_repo_clone_url)
  pct exec "$DB_CTID" -- bash -c "
    set -e
    cd /opt
    rm -rf spp-sql

    git clone --depth 1 \"$repo_url\" spp-sql
  "
}
update_sql_repo() {
  pct exec "$DB_CTID" -- bash -c "
...
shared_config_menu() {
  echo
  echo "Configuration (Shared Services)"
  echo "Launcher realmlist actions here are install/repair tools."
  echo
...
edit_vmangos_instance_names() {
  ...
  echo
  echo "Current vMaNGOS game instances:"
  printf '  %s\n' "${GAME_INSTANCE_NAMES[@]}"
  echo
  echo "The first listed instance owns the vMaNGOS realmd flow."
  echo

  local new_base new_count
  read -p "Base name [${VMANGOS_CONTAINER_BASENAME:-vmangos}]: " new_base
  read -p "Instance count [${VMANGOS_INSTANCE_COUNT:-1}]: " new_count
  new_base="${new_base:-${VMANGOS_CONTAINER_BASENAME:-vmangos}}"
  new_count="${new_count:-${VMANGOS_INSTANCE_COUNT:-1}}"

  if [[ ! "$new_count" =~ ^[0-9]+$ || "$new_count" -lt 1 ]]; then
    echo "Instance count must be a positive number."
    read -p "Press Enter to continue..." _
    return 1
  fi

  VMANGOS_CONTAINER_BASENAME="$new_base"
  VMANGOS_INSTANCE_COUNT="$new_count"
  GAME_INSTANCE_NAMES=()
  local i
  for ((i=1; i<=VMANGOS_INSTANCE_COUNT; i++)); do
    GAME_INSTANCE_NAMES+=("$(default_vmangos_target_name "$i")")
  done
  VMANGOS_REALM_OWNER="${GAME_INSTANCE_NAMES[0]}"

  set_config_value "VMANGOS_CONTAINER_BASENAME" "$VMANGOS_CONTAINER_BASENAME"
  set_config_value "VMANGOS_INSTANCE_COUNT" "$VMANGOS_INSTANCE_COUNT"
  set_config_value "VMANGOS_REALM_OWNER" "$VMANGOS_REALM_OWNER"
  persist_array_config_line "GAME_INSTANCE_NAMES" "${GAME_INSTANCE_NAMES[@]}"

  persist_config_storage || {
    echo "Failed to save vMaNGOS instance metadata."
    read -p "Press Enter to continue..." _
    return 1
  }
  ...
  echo
  echo "Saved vMaNGOS game instances:"
  printf '  %s\n' "${GAME_INSTANCE_NAMES[@]}"
  echo "Realm owner: ${VMANGOS_REALM_OWNER}"
  read -p "Press Enter to continue..." _
}
...
cd /opt
rm -rf spp-settings
git clone --depth 1 --filter=blob:none --sparse \"$repo_url\" spp-settings
cd spp-settings
...
shared_website_menu() {
  echo
  echo "Website (Shared Admin Surface)"
